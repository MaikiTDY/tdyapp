 ...
 *******************************/
public class HotelHeadView extends BaseView {
    ...
    public HotelHeadView(Context context, List<HotelBean> beans) {
        super(context);
        this.beans = beans;
        initView();
        initDate();
    ...
    public void initDate() {
        ...
        if (beans!=null && beans.size()>0) {
            int size = beans.size();
            imageLoader.loadImage(beans.get(size - 1).url, imageView);
            title.setText(beans.get(size - 1).name);
        }
    }
